...
\rule{\linewidth}{0.4pt}
        ...
        \[\varphi = \sum_{k=1}^N c_k \chi_{E_k}, \quad \bigsqcup_{k=1}^N E_k. = E,\] where each $E_k$ is measurable. By the lemma above, for each $E_k,$ there exists a compact set $X_k \subseteq E_k$ such that 
        \[m(E_k \sm X_k) < \frac{1}{N n}\] Each $X_k$ is measurable and since it is a subset of disjoint sets, then the $X_k$ are pairwise disjoint. Define
        \[X = \bigsqcup_{k=1}^N X_k.\] 
        Moreover, note that $X$ is measurable since it is the countable union of measurable sets. We claim that 
        \begin{enumerate}
            \item $\varphi$ is continuous on $X$
            \item $m(E\sm X) \leq \frac{1}{n}.$
        \end{enumerate}
        To see (i), let $x\in X$ let $\eta >0.$ Then $x \in X_k$ for some $k$ and $\varphi(x) = c_k.$ Since the $X_k$ are compact and disjoint, 
        \[d(X_k, X_{k+1}) =: r_k >0\] is achieved and is positive for each $k.$ Thus, we take $\delta< \min\{r_k\}_k$ If $y\in (x - \delta, x + \delta),$ then, $y \in X_k,$ and so $\varphi(y) = c_k.$ Thus, \[|\varphi(y) - \varphi(x)| = 0 < \eta.\] Thus, $\varphi|_{X}$ is continuous.
        To see (ii), consider that 
        \begin{align*}
            m(E\sm X) &= m(E\sm \bigsqcup_{k=1}^NX_k)\\
            &= m(\bigsqcup_{k=1}^N E_k \sm \bigsqcup_{k=1}^N X_k)\\
            &= m(\bigsqcup_{k=1}^N E_k \sm X_k)\\
            &= \sum_{k=1}^N m(E_k \sm X_k)\\
            &< \sum_{k=1}^N \frac{1}{Nn}\\
            &= \frac{1}{n}\\
            &< \epsilon
        \end{align*}
Since $X$ is the finite union of closed sets, then $X$ is closed. We use the lemma provided in the PSET to conclude that since $\varphi$ is continuous on the closed $X,$ then $\varphi$ has a continuous extension $g$ on $\bbR$ such that $\varphi = g$ on $X.$ 
    \end{solution}
